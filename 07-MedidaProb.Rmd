...
fn=c(0,0.5,1,0.5,0)
ggplot() + theme_bw() +
  geom_line(data=tibble(w=theta,X=dpost),aes(x=w,y=X)) +
  geom_segment(aes(x=En[-length(En)],xend=En[-1],y=fn,yend=fn),size=1.5) +
  geom_segment(aes(x=En[-length(En)],xend=En[-1],y=0,yend=0,colour=as.factor(c(0,1,2,1,0))),size=1.5) +
...
(iii) *Singular* se $F_s$ é contínua com $F_s'=0~$ $\lambda$-q.c. ($F_s$ é singular com relação à medida de Lebesgue).

**Resultado:** Seja $F$ uma f.d. Então, $F$ pode ser escrita como $F=\alpha_1F_d+\alpha_2F_c+(1-\alpha_1+\alpha_2)F_s$, com $\alpha_1,\alpha_2\geq 0$ tal que $\alpha_1+\alpha_2\leq 1$. 
