...
+ Uma coleção de eventos $A_1,A_2,\ldots$ forma uma *partição* de $\Omega$ se $A_i \cap A_j = \emptyset$, $\forall i \neq j$, e $\displaystyle \bigcup_{i=1}^{\infty} A_i = \Omega$.
...
w2 = A/(A+B)
post = w2*dbeta(theta,a1+x,b1+k-x)+
...
En<-list()         
for(i in 1:length(w)){
  En[[i]]<-rep(0,length(post))
}
for(l in 1:(length(En))){
  for(j in 1:(length(w[[l]])-1)){ 
    for(i in 1:length(post)){
      if(post[i]<w[[l]][j+1]&post[i]>=w[[l]][j]){
        En[[l]][i]<-w[[l]][j]
      }else{
        En[[l]][i]<-En[[l]][i]
      }
...
tibble(x=as.factor(rep(seq(1,n),each=length(theta))),
       En = unlist(En), theta = rep(theta,n), post = rep(post,n)) %>% 
  ggplot() + 
...
Suponha agora $X$ v.a. absolutamente contínua com densidade $f_X$. Nesse caso, $P_X(X\in A)$ $=\displaystyle\int_Af_X(t)dt$ $=\displaystyle\int_{\mathbb{R}}\mathbb{I}_A(t)f_X(t)dt$. Vamos "verificar" que $E\left[g(X)\right]$ $=\displaystyle\int_{\mathbb{R}} g(x)f(x)dx$.
...
Ainda neste exemplo, considere $g:\mathbb{R}\rightarrow\mathbb{R}$ com $g(t) = t^c(1-t)^d$, com $c,d>0$ fixados. Então,
...
* É possível usar uma f.d. $F$ para criar uma medida em $(\mathbb{R},\mathcal{B}(\mathbb{R}))$. Para tal, defina $P\left((a,b]\right)=F(b)-F(a)$ e extenda essa medida para a $\sigma$-álgebra usando o teorema de extensão de Caratheodory (veja, por exemplo, @Schervish12, pág. 578).
...
* Neste caso, se $g: \mathbb{R}\longrightarrow \mathbb{R}$ é uma função mensurável, não será feita distinção entre $\int g(x)dF(x)=$ $\int g(x)dP(x)$.

* Se $P$ é uma probabilidade em $(\mathbb{R}^k,\mathcal{B}(\mathbb{R}^k))$ então $F(x_1,\ldots,x_k)=$ $P((-\infty,x_1]\times \ldots\times (-\infty,x_k])$ é a *função de distribuição conjunta* do *vector aleatório* $\boldsymbol{X} = (X_1,\ldots,X_K)$.

$~$


**Definição:** $(\Omega, \mathcal{A}, P)$ espaço de probabilidade e $(\mathfrak{X},\mathfrak{F},\mathcal{V})$ espaço mensurável. Considere $X: \Omega \longrightarrow \mathfrak{X}$ uma v.a. e $P_X$ a medida induzida por $X$ de $P$, i.e. $P_X(B) = P(X^{-1}(B)).$ Suponha que $P_X << \mathcal{V}$. Então, a derivada de Radon-Nikodin $f_X = \dfrac{d\mu_X}{d\mathcal{V}}$ é a densidade de $X$ com respeito a $\mathcal{V}$.
...
**Resultado:** $X_1,X_2,\ldots,X_k$ são v.a. independentes com $E(X_i)<\infty,$ $i=1,\ldots,k$
