...
Se $D(\omega_1)$ denota uma secção de $D$ em $\omega_1,$ isto é, $D(\omega_1)=$ $\{\omega_2\in \Omega_2: (\omega_1,\omega_2)\in D\},$ $D\in \mathcal{A}=\mathcal{A}_1\times\mathcal{A}_2,$ então

$P(D)$ $=\displaystyle\int_{\Omega_1} \mu\left(\omega_1,D(\omega_1)\right)dP_1(\omega_1).$
...
Se $C=\{(x,y):~ x\in A, y \in B\}$ $=A\times B~,$ $C(x)=B$ se $x\in A$ e $C(x)=\varnothing$ se $x \notin A~,$ então
...
**Exemplo:** Se $X \sim Beta(a,b)$ e $Y|X=x \sim Bin(n,x)$ 

$\left(\Omega_1=[0,1],\mathcal{A}_1=\mathcal{B}([0,1]),P_X\right),$ onde

$P_X(A)$  $=\displaystyle\int_{\mathbb{R}}\mathbb{I}_A dF_X(x)$ $=\displaystyle\int_A f_X(x)dx$ $=\displaystyle\int_A \dfrac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}x^{a-1}(1-x)^{b-1}dx$ 

para $A \in \mathcal{A}_1.$ 

Além disso, considere $\left(\Omega_2=\{0,1,\ldots,n\}, \mathcal{A}_2=\mathcal{P}(\Omega_2)\right)$ e, para cada $x \in [0,1],$ $\mu(x,B)=P(Y \in B| X=x).$ Então, pra $k=0,1,\ldots,n,$ 

$\mu(x,B)$ $=P(Y\in B| X=x)$ $=\displaystyle\binom{n}{k}x^k(1-x)^{n-k}~$ (que é mensurável em $x$). 

Tomando $\Omega=\Omega_1 \times \Omega_2,$ $\mathcal{A}=\mathcal{A}_1 \times \mathcal{A}_2,$ $P$ é a única medida de probabilidade determinada por $P_X$ (ou $F_X$) e $\mu(x,\cdot)~.$

$P(C)$ $=\displaystyle\int_{\Omega_1}\mu\left(x,C(x)\right)dP_X$ $=\displaystyle\int_0^1 \mu\left(x,C(x)\right)dF_X(x)$ $=\displaystyle\int_0^1 \mu\left(x,C(x)\right)f_X(x)dx,$ $C \in \mathcal{A}.$

$~$

Por exemplo, se $C=\Omega_1 \times \{k\},$ temos

$P_Y\left(Y=k\right)$ $=P\left(\Omega_1 \times \{k\}\right)$ 
$=P\left(\left\{X\in[0,1]\right\}~,~\{Y=k\}\right)$ 
$\int_0^1P(Y=k|X=x)dF(x)=$ $\int_0^1 \binom{n}{k}x^k(1-x)^{n-k}$ $\dfrac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}x^{a-1}(1-x)^{b-1}dx=$ $\dfrac{\binom{n}{k}\dfrac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}}{\dfrac{\Gamma(a+b+n)}{\Gamma(a+k)\Gamma(b+n-k)}}$ $\int_0^1\dfrac{\Gamma(a+b+n)}{\Gamma(a+k)\Gamma(b+n-k)}$ $x^{a+k-1}(1-x)^{b+n-k-1}dx=$ $\binom{n}{k}\dfrac{\Gamma(a+b)}{\Gamma(a)\Gamma(b)}\dfrac{\Gamma(a+k)\Gamma(b+n-k)}{\Gamma(a+b+n)}.$
...
**Exemplo1:** $\mathfrak{X}\{x_1,x_2,\ldots\}$ com $p_i=P(\{X=x_i\})>0.$ Defina 
