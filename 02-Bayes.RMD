...
**Exemplo.** No gráfico a seguir, a distribuição a priori é uma $\text{Pareto}(2,1)$ e foi observada uma amostra de tamanho $5$ com $x_{(n)}=2$, de modo que a posteriori é $\text{Pareto}(7,2)$.

```{r ParetoConjugatePrior}
a=2;b=1
n=5; xn=2
...
**Exemplo.** Considere que a distribuição a priori é uma $\text{Gama}(2,2)$ e foi observado $n=4$, $\sum x=1$, $m=5$ e $\sum y=10$, de modo que a posteriori é $\text{Gama}(16,9)$.

```{r GammaConjugatePrior}
a=2;b=2
n=4; sumx=1
m=6; sumy=10
t = seq(0,5,length.out=1000)
...
**Exemplo 1.**  Seja $X|\theta \sim Bin(n,\theta)$ e $f(\theta)$ $=wf_1(\theta)+(1-w)f_2(\theta)$, com $f_1\sim Beta(a_1,b_1)$ e $f_2\sim Beta(a_2,b_2)$.
...
**Exemplo 2.** Suponha que $\theta$ é uma proporção e você deseja construir uma distribuição a priori baseada em estudos anteriores. Como, em geral, os dados de outros estudos não estão disponíveis na íntegra, você pode utilizar estatísticas resumo.
...
**Proposta 1.** Uma primeira forma de fazer isso seria supor que, antes de observar esses estudos, você não tem informação sobre $\theta$, e usar a posteriori obtida com os dados do estudos como priori. Assim, seja $X_i|\theta \sim \text{Bin}(n_i,\theta)$, $i=1,2,3$, e $\theta \sim \text{Uniforme}(0,1)$. Temos que
...
**Proposta 2.** Considere agora que a priori será construída da seguinte forma: para cada estudo será calculada uma "posteriori" supondo que $\theta \sim \text{Uniforme}(0,1)$ e faremos uma mistura dessas posterioris ponderada pelo tamanho amostral dos estudos. Assim:
...
<!-- post = function(theta){ -->
<!--   dmixbeta(theta,wx,ax,bx) -->
<!-- } -->

<!-- modapost = optimize(post,c(0,1),maximum = TRUE)$maximum -->

<!-- hmax = optimize(post,c(0,1),maximum = TRUE)$objective -->
<!-- hmin=0 -->
<!-- h=(hmin+hmax)/2 -->
<!-- li = uniroot(function(t){post(t)-h},c(0,modapost))$root -->
<!-- ls = uniroot(function(t){post(t)-h},c(modapost,1))$root -->
<!-- p=pmixbeta(ls,wx,ax,bx)-pmixbeta(li,wx,ax,bx) -->

<!-- while(p>gama+eps | p<gama-eps){ -->
<!--   if(p<gama+eps){ -->
<!--     hmax=h -->
<!--     h=(hmax+hmin)/2 -->
<!--   } else{ -->
...
<!--        pos=post(theta)) %>%  -->
<!--   ggplot() + theme_bw() + -->
<!--     geom_line(aes(theta,pos))+ -->
<!--     geom_hline(yintercept = h,linetype=2,colour="darkgrey")+ -->
<!--     geom_segment(x=li,xend=li,y=0,yend=post(li),color="red",linetype=2)+ -->
<!--     geom_segment(x=ls,xend=ls,y=0,yend=post(ls),color="red",linetype=2)+ -->
<!--     geom_segment(x=li,xend=ls,y=0,yend=0,color="red",lwd=2) -->

<!-- cat("IC = [",li," ; ",ls,"]") -->
<!-- ``` -->

<!-- ```{r IC_simul} -->
<!-- t = rmixbeta(10000,wx,ax,bx) -->
<!-- ft = post(t) -->

<!-- corte = quantile(ft,1-gama) -->

<!-- #t[ft==corte] -->

<!-- lis = uniroot(function(t){post(t)-corte},c(0,modapost))$root -->
<!-- lss = uniroot(function(t){post(t)-corte},c(modapost,1))$root -->

<!-- tibble(theta=seq(eixox[1],eixox[2],length.out=1000), -->
<!--        pos=post(theta)) %>%  -->
<!--   ggplot() + theme_bw() + -->
<!--     geom_line(aes(theta,pos))+ -->
<!--     geom_hline(yintercept = corte,linetype=2,colour="darkgrey")+ -->
<!--     geom_segment(x=lis,xend=lis,y=0,yend=post(lis),color="red",linetype=2)+ -->
<!--     geom_segment(x=lss,xend=lss,y=0,yend=post(lss),color="red",linetype=2)+ -->
<!--     geom_segment(x=lis,xend=lss,y=0,yend=0,color="red",lwd=2) -->

<!-- cat("IC_simul = [",lis," ; ",lss,"]") -->
<!-- ``` -->
