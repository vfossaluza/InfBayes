...
```{r}
new_data <- data.frame(Vendas=seq(dados$Vendas[108]-mAntes[1],-0.4,length.out=12)+c(0,rnorm(11,0,0.08)))
...
```{r}
new_data <- data.frame(Vendas=seq(dados$Vendas[108]-mAntes[1],-1.1,length.out=12)+c(0,rnorm(11,0,0.08)))
pred1 <- predict(fit1, new_data)
plot_predict(pred1, alpha=0.8) +
  scale_x_continuous(breaks=seq(1,length(dados$Período)+1,12),labels=c(unique(year(dados$Período)),2017))
```
...
<!-- fit2 <- dados %>%  -->
<!--   mutate(Vendas=Vendas-mAntes[1],Resistência=Resistência-mAntes[2]) %>% -->
<!--   walker(data=., formula = Resistência ~ -1+rw1(~ Vendas,beta=c(0,100), -->
<!--     sigma=c(0,100)),sigma_y_prior = c(0,100), chain=2) -->
<!--         #Default: chain=4, iter=2000, warmup=1000, thin=1 -->

<!-- multiplot( -->
<!--   mcmc_areas(as.matrix(fit2$stanfit), regex_pars = c("sigma_rw1"), prob=0.9), -->
...
<!-- new_data <- data.frame(Vendas=seq(-0.81441839,-0.4,length.out=12)+c(0,rnorm(11,0,0.1))) -->
<!-- pred2 <- predict(fit2, new_data) -->
<!-- plot_predict(pred2, alpha=0.8) + -->
<!--   scale_x_continuous(breaks=seq(1,length(dados$Período)+1,12),labels=c(unique(year(dados$Período)),2017)) -->

<!-- new_data <- data.frame(Vendas=seq(-0.81441839,-1,length.out=12)+c(0,rnorm(11,0,0.1))) -->
<!-- pred2 <- predict(fit2, new_data) -->
<!-- plot_predict(pred2, alpha=0.8) + -->
<!--   scale_x_continuous(breaks=seq(1,length(dados$Período)+1,12),labels=c(unique(year(dados$Período)),2017)) -->
<!-- ``` -->
